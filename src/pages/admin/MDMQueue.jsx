  ...
  return (
    <Modal title="Tolak Pengajuan" subtitle={`${MDM_TYPES[entry.type]?.label} — ${entry.data?.name || entry.data?.inv_no}`} onClose={onClose}>
      <div className="p-5 space-y-4">
        <div>
          <label className="erp-label">Alasan Penolakan <span className="text-red-400">*</span></label>
          <textarea
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          pendingCount > 0 && (
            <span className="flex items-center gap-2 bg-amber-500/20 border border-amber-500/30 text-amber-300 text-sm font-bold px-3 py-1.5 rounded-full">
        ...
        <div className="flex gap-1 bg-gray-900 p-1 rounded-lg">
          {FILTERS.map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-1.5 text-xs rounded font-medium capitalize transition-colors ${filter === f ? "bg-blue-600 text-white" : "text-gray-400 hover:text-gray-200"}`}>
              {f === "all" ? "Semua" : f === "pending" ? `Menunggu ${pendingCount > 0 ? `(${pendingCount})` : ""}` : f === "approved" ? "Disetujui" : "Ditolak"}
            </button>
          ))}
        </div>
        <div className="flex gap-1 bg-gray-900 p-1 rounded-lg flex-wrap">
          <button onClick={() => setTypeFilter("all")}
            className={`px-3 py-1.5 text-xs rounded font-medium transition-colors ${typeFilter === "all" ? "bg-gray-700 text-white" : "text-gray-400 hover:text-gray-200"}`}>
            Semua Tipe
          </button>
          {Object.entries(MDM_TYPES).map(([k, v]) => (
            <button key={k} onClick={() => setTypeFilter(k)}
              className={`px-3 py-1.5 text-xs rounded font-medium transition-colors ${typeFilter === k ? "bg-gray-700 text-white" : "text-gray-400 hover:text-gray-200"}`}>
              {v.icon} {v.label}
            </button>
          ))}
        </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 flex-wrap">
                      <div>
                        ...
                        <h3 className="text-base font-black text-white mt-0.5">{name}</h3>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-1.5 text-xs text-gray-400">
                      <span>Diajukan oleh <strong className="text-gray-300">{entry.requestedBy?.name}</strong> ({entry.requestedBy?.role})</span>
                      <span>{DATE(entry.requestedAt)}</span>
                      {entry.processedBy && (
                        <span>
                          {entry.status === "approved" ? "Disetujui" : "Ditolak"} oleh <strong className="text-gray-300">{entry.processedBy.name}</strong> · {DATE(entry.processedAt)}
                        </span>
                      )}
                    </div>
