  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        ...
        <KPICard label="Inventory Value"   value={IDR(totalValue)}    icon="💰" color="text-green-700" />
        <KPICard label="Low Stock Alerts"  value={lowStock}           icon="⚠️" color={lowStock?"text-red-700":"text-gray-900"} />
      ...
      {[["🪵 Finished Goods — Plywood", plywood], ["🧪 Raw Materials & Chemicals", raw]].map(([title, items])=>(
        <div key={title} className="erp-card mb-5">
          <div className="px-5 py-3 border-b border-gray-200"><h3 className="font-bold text-sm text-gray-700">{title}</h3></div>
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td className="text-right">
                        <span className={`font-black ${low?"text-red-700":"text-gray-900"}`}>{NUM(p.stock_qty)}</span>
                      ...
                      <td>
                        {low
                          ? <span className="text-xs font-bold text-red-700 bg-red-500/10 px-2 py-0.5 rounded">Low Stock</span>
                          : <span className="text-xs font-bold text-green-700 bg-green-500/10 px-2 py-0.5 rounded">OK</span>
                        }
                      </td>
