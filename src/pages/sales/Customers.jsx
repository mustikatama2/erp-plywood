...
export default function Customers() {
  ...
  return (
    <div>
      ...
      {/* Non-admin info */}
      {!isAdmin && !canSubmit && (
        <div className="mb-4 flex items-center gap-2 bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-sm text-gray-400">
          <span>🔒</span>
      ...
      <Card>
        ...
        <Table onRowClick={setSelected} columns={[
          ...
          { key:"open_invoices",label:"Invoice", right:true, render:v=>v>0?<span className="text-amber-400 font-bold">{v}</span>:<span className="text-gray-600">0</span> },
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="erp-card p-3 text-center">
                <p className={`text-2xl font-black ${(selected.ar_balance||0)>0?"text-amber-300":"text-gray-400"}`}>
                  {selected.currency} {(selected.ar_balance||0).toLocaleString()}
                </p>
                <p className="text-xs text-gray-500 mt-1">Piutang Belum Diterima</p>
              </div>
              <div className="erp-card p-3 text-center">
                <p className="text-2xl font-black text-white">{selected.total_so || 0}</p>
